...
// 1.1 ) Definition of the studied period
  ...
  var months = ee.List.sequence(1, 12);//this is the growing season in Costa Rica

// 1.2) Select Image Collection // Do not modify this section
  var coll1 = ee.ImageCollection('MODIS/006/MOD13Q1').filterDate(String(FirstYear)+'-01-01', String(LastYear)+'-12-31'); 
...
var getPrincipalComponents = function(centered, scale, region) {
  ...
  description: 'eigenValues_MODIS_CR_2001_2017',
  folder: GDriveOutputImgFolder,
  fileFormat: 'CSV'
  });
  
  // Export.table.toDrive(eigenValues,'eigenValues', {fileFormat: "csv"});
  // This is a PxP matrix with eigenvectors in rows.
  var eigenVectors = eigens.slice(1, 1);
  print('eigenVectors',ee.FeatureCollection(eigenVectors));
...
Export.image.toAsset({
      ...
      description: 'MODIS_2001_2017_PC1_A',
      assetId: 'MODIS_2001_2017_PC1_A',
      scale: scale,
      region: costa_rica,
      maxPixels: 1e13,
    }) ;
...
Export.image.toDrive({
      image: pcImage.select('pc1').clip(region),
      description: 'MODIS_2001_2017_PC1',
      region: costa_rica,
      maxPixels: 1e13,
      folder: GDriveOutputImgFolder,
      scale: scale
...
Export.image.toAsset({
      ...
      description: 'MODIS_2001_2017_PC2_A',
      assetId: 'MODIS_2001_2017_PC2_A',
      scale: scale,
      region: costa_rica,
      maxPixels: 1e13,
    }) ;
...
Export.image.toDrive({
      image: pcImage.select('pc2').clip(region),
      description: 'MODIS_2001_2017_PC2',
      region: costa_rica,
      maxPixels: 1e13,
      folder: GDriveOutputImgFolder,
      scale: scale
...
Export.image.toAsset({
      ...
      description: 'MODIS_2001_2017_PC3_A',
      assetId: 'MODIS_2001_2017_PC3_A',
      scale: scale,
      region: costa_rica,
      maxPixels: 1e13,
    }) ;
...
Export.image.toDrive({
      image: pcImage.select('pc3').clip(region),
      description: 'MODIS_2001_2017_PC3',
      region: costa_rica,
      maxPixels: 1e13,
      folder: GDriveOutputImgFolder,
      scale: scale
