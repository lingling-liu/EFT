var point = /* color: #d63000 */ee.Geometry.Point([-84.1153564453125, 10.083754913473248]),
    PC1 = ee.Image("users/lliu6/Landsat8_PC1"),
    L8_EVI = ee.ImageCollection("LANDSAT/LC8_L1T_8DAY_EVI"),
    L7_EVI = ee.ImageCollection("LANDSAT/LE7_L1T_8DAY_EVI"),
    L5_EVI = ee.ImageCollection("LANDSAT/LT5_L1T_8DAY_EVI"),
    test_ROI = /* color: #d63000 */ee.Geometry.Polygon(
        [[[-84.403037140625, 9.861914537133636],
          [-83.84273440625, 9.49369595605381],
          [-83.590048859375, 10.089139313979702],
          [-84.3920508125, 10.208098089683372]]]);
...
//var TFE =  ee.ImageCollection.fromImages([Media, MMax, Season])
//var TFE = Media.addBands([Season,browning_time])
...
var PPN = Media.where(Media.lte(Mp20), 100);
PPN = PPN.where(Media.gt(Mp20).and(Media.lte(Mp40)),200);
PPN = PPN.where(Media.gt(Mp40).and(Media.lte(Mp60)), 300);
PPN = PPN.where(Media.gt(Mp60).and(Media.lte(Mp80)), 400);
PPN = PPN.where(Media.gt(Mp80), 500);
...
var Seasonality = Season.where(Season.lte(Season33), 10);
Seasonality = Seasonality.where(Season.gt(Season33).and(Season.lte(Season66)),20); 
Seasonality = Seasonality.where(Season.gt(Season66), 30); 

var browning_time = Min.select(['NDVI_min']);
var Feno = browning_time.where(browning_time.lte(0.28), 1); //March April
Feno = Feno.where(browning_time.gt(0.28).and(browning_time.lte(0.48)),2); //May Jun
Feno = Feno.where(browning_time.gt(0.48), 3); //Aug Sep

var TFEcat = PPN.int().addBands([Seasonality,Feno]).int(); 
var TFEcat2 = Seasonality.int(); 

var TFEunib = TFEcat.reduce(ee.Reducer.sum());
var TFEunib2 = TFEcat2.reduce(ee.Reducer.sum());

//EFTs from 1 to 45
var clasInpt = ([111, 112, 113, 121, 122, 123,
131, 132, 133, 211, 212, 213, 221, 222, 223, 
231, 232, 233, 311, 312, 313, 321, 322, 323, 
331, 332, 333, 411, 412, 413, 421, 422, 423, 
431, 432, 433, 511, 512, 513, 521, 522, 523,
531, 532, 533]);

var clasInpt2 = ([10, 20, 30]);

var clasesF = ee.List.sequence(1, 45);
var clasesF2 = ee.List.sequence(1, 3);

var TFEclas = TFEunib.remap(clasInpt, clasesF);
var TFEclas2 = TFEunib2.remap(clasInpt2, clasesF2);

var TFEunib = TFEunib.toUint16(); 
var TFEunib2 = TFEunib2.toUint16(); 

print('TFE111-533',TFEunib);
print('TFE1-45',TFEclas);
...
Map.addLayer(TFEclas, vizParamsTFEclasArctic, 'TFE1-64');
    ...
    Export.image.toDrive({
      image: TFEclas,
      description: 'EFT_Landsat7_EVI_2001_2017',
      region: costa_rica,
      maxPixels: 1e13,
      folder: GDriveOutputImgFolder,
      scale: scale
    ...
    Export.image.toDrive({
      image: TFEclas2,
      description: 'EFT_Landsat7_EVI_2001_2017_2nd',
      region: costa_rica,
      maxPixels: 1e13,
      folder: GDriveOutputImgFolder,
      scale: scale
