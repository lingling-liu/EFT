...
// 1.2) Select Image Collection // Do not modify this section
  ...
  //var coll1 = ee.ImageCollection('LANDSAT/LC8_L1T_32DAY_EVI').filterDate(String(FirstYear)+'-01-01', String(LastYear)+'-12-31'); // EVI y NDVI
print('coll1',coll1)
...
image: stacked,
description: 'MODIS_yearly_average'+filename_add ,
maxPixels: 1e13,
...
//*****************************************************
//calculate 16-day NDVI composites for growing season
//*****************************************************
...
//second MMin
var Season = Min.select(['t1']);
var Seasonality = Season.where((Season.gt(1).and(Season.lte(81))).or((Season.gt(337).and(Season.lte(365)))), 10);
Seasonality = Seasonality.where(Season.gt(81).and(Season.lte(225)),20); 
Seasonality = Seasonality.where(Season.gt(225).and(Season.lte(337)),30); 

// greening_time
var Feno = greening_time.where((greening_time.gt(1).and(greening_time.lte(113))).or((greening_time.gt(321).and(greening_time.lte(365)))), 1);
Feno = Feno.where(greening_time.gt(113).and(greening_time.lte(177)),2); 
Feno = Feno.where(greening_time.gt(177).and(greening_time.lte(321)),3);
...
Map.addLayer(TFEclas, vizParamsTFEclasArctic, 'TFE1-64');
    ...
    Export.image.toDrive({
      image: TFEclas,
      description: 'EFT_MODIS_EVI_2001_2017',
      region: costa_rica,
      maxPixels: 1e13,
      folder: GDriveOutputImgFolder,
      scale: scale
