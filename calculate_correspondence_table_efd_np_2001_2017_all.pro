pro calculate_correspondence_table_EFD_NP_2001_2017_all
 
  ;EFT NP from CR
  infile="/largedisk_a/Costa_Rica/EFT/EFD_Landsat7_EVI_2001_2017_4_4_window_size_3_clp.tif"
  ...
  index = where(ESA_data ge 1 and ESA_data le 9,count1)
  
 
  ;EFT NP
  infile="/largedisk_a/Costa_Rica/EFT/EFD_Landsat7_EVI_2001_2017_NP_4_4_window_size_3.tif"
  IMN_data = READ_TIFF(infile,geotiff = geotiff)
  ns2 = n_elements(IMN_data[*,0])
  nl2 = n_elements(IMN_data[0,*])
  print,ns2,nl2
  
  index = where(IMN_data ge 1 and IMN_data le 9, count2)
  print,count1, count2
  ...
  index = where(IMN_data ge 1 and IMN_data le 9,count_IMN)
  
  result = lonarr(10,9)+32767
  result1 = fltarr(10,9)+32767
  for i=1,9 do begin
    print,i
    index = where(ESA_data eq i, count)
    temp = IMN_data[index]
       
    index1 = where(temp ge 1 and temp le 9,count1)
  ...
  WRITE_CSV,'/largedisk_a/Costa_Rica/EFT/EFD_b4_w3_2001_2017_correspondence_table_NP_based_national_scale_percentages_all.csv',result1
end
